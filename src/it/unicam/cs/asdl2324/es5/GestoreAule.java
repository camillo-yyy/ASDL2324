...
public class GestoreAule {
    ...
    public boolean addAula(Aula a) {
        if(a == null) throw new NullPointerException("Oggetto nullo");

        if(this.aule.add(a)) return true; // Se non esiste nell'insieme aggiunge e ritorna true
        else return false;
    }
    ...
    public Set<Aula> cercaAuleLibere(Set<Facility> requestedFacilities,
        ...
        if(ts == null || requestedFacilities == null) throw new NullPointerException("Oggetto nullo");    

        Set<Aula> temp = new HashSet<Aula>();

        for (Aula a : aule) {

            if(a.satisfiesFacilities(requestedFacilities) && a.isFree(ts)) temp.add(a);
