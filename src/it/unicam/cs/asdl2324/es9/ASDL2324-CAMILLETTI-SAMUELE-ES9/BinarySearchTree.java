    ...
    protected class RecBST {
        ...
        protected int computeHeight() {
            ...
            // 2 casi ricorsivi
                ...
                // calcolo ricorsivamente l'altezza dei sotto alberi
                if(this.left != null) heightLeft = left.computeHeight() + 1; 
                if(this.right != null) heightRight = right.computeHeight() + 1;
            }

            // ritorna solo l'altezza maggiore tra i due
        ...
        protected boolean insert(E label) {
            ...
            // se trovo l'elemento, significa che è già presente
            if(label.compareTo(this.label) == 0) return false;
            else if(label.compareTo(this.label) < 0) { // se è minore sotto albero di sinistra
                // caso base (lo inserisco)
                if(this.left == null) this.left = new RecBST(label, null, null, this);
                // caso ricorsivo
                else this.left.insert(label);
                return true;
            }
            else { // se è maggiore sotto albero di sinistra
                // caso base (lo inserisco)
                if(this.right == null) this.right = new RecBST(label, null, null, this);
                // caso ricorsivo
                else this.right.insert(label);
                return true;
            }
        }
        ...
        protected RecBST search(E label) {
            ...
            // se trovo l'elemento, significa che è già presente
            if(label.compareTo(this.label) == 0) return this;
            else if(label.compareTo(this.label) < 0) { // se è minore sotto albero di sinistra
                // caso base
                if(this.left == null) return null;
                // caso ricorsivo
                ret = this.left.search(label);
            }
            else { // se è maggiore sotto albero di sinistra
                // caso base
                if(this.right == null) return null;
                // caso ricorsivo
                ret = this.right.search(label);
            }
        ...
        protected void addLabelsInOrder(List<E> l) {
            ...
            // visita in-order sfruttando la tecnica DFS in modo simmetrico
            if(this.left != null) this.left.addLabelsInOrder(l);
            l.add(this.label);
            if(this.right != null) this.right.addLabelsInOrder(l);
        ...
        protected List<E> inOrderVisit() {
            ...
            List<E> l = null;
            // caso ricorsivo
            if(this.left != null) l = this.left.inOrderVisit();
            // caso base
            if(this.left == null) l = new ArrayList<E>();

            l.add(this.label); // aggiungo alla lista

            // sotto albero destro
            if(this.right != null) l.addAll(this.right.inOrderVisit()); // chiamo ricorsivamente sul sotto albero destro
        ...
        protected RecBST getSuccessorNode() {
            ...
            if(this.right == null) { 
                ...
                // se non ha un figlio destro controllo il primo nodo minore tra gli antenati (se li ha)
                if(this.parent != null) {

                    RecBST node = this;
                    RecBST temp = this.parent;

                    // salgo verso l'alto
                    while(temp.parent != null && temp.label.compareTo(node.label) > 0) {
                        temp = temp.parent;
                    }   
                    
                    if(temp.label.compareTo(node.label) < 0) return temp;
                    else return null;

                }
                else return null;
            }
            else return this.right.getMinNode(); // Il minimo del figlio destro è il successore per definizione del bst
        ...
        protected RecBST getPredecessorNode() {
            ...
            if(this.left == null) {
                 // se non ha un figlio destro controllo il primo nodo maggiore tra gli antenati (se li ha)
                if(this.parent != null) {
            ...
            else return this.left.getMaxNode(); // Il maggiore del figlio sinistro è il predecessore per definizione del bst
        ...
        protected void deleteSelfLabel() {
            RecBST succ = this.getSuccessorNode(); // vedo se ha un successore

            if(succ == null) {
                if(this.parent == null) {
                    BinarySearchTree.this.root = null; // Se è la testa la rimuovo
                    return;
                }

                if(this.parent.right == this) this.parent.right = null; // Altrimenti se è  il figlio destro del padre
                else this.parent.left = null; // Se è il figlio sinistro del padre

                this.parent = null; // scollego
